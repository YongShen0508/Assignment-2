...
struct mapping_seats{//cinema hall	//a set of column and row = 1 data
		...
		do {
			...
			for (int i = 0; i < unavailable[number - 1].data; i++){
				if (row > 10 || column > 14){
					...
					found++;
					string confirms;
					cout << "\t\t\tAre you confirm delete row " << row << " and " << "column " << column << " <Y>es to delete...  >>";
					cin >> confirms;
					confirms = upper(confirms);					
					if (confirms == "Y"){
...
{//storage is i value from modication section
	...
	for (int i = 0; i < record; i++)
	{
		...
		if (i == storage){		//directly enter next round looping
			detection++;	
			continue;
		}
						...
						do {
							...
							if (booking == "Y"){
								...
								if (final == "Y"){
									...
									cout << "\t\t\t\tPrice After Tax \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t" << total_price * service_tax + total << endl;
